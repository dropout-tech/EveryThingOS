...
產品外殼在 `web/`（Next.js）。背景是一條從海面到海底的連續風景。介面是少數幾塊可折射的玻璃。識別是玻璃水滴加漣漪。

打開先問你做哪一行。選了就進「今天」：一件事、一個人、一個按鈕。沒有行銷導覽、沒有激勵句、沒有假的本月營收看板。
...
- `/industries` 一百個產業包
- `/workspace` 今天這一件
- `/workspace/erp` 生意：同一張單往下走
- `/workspace/erp/compare` 和鼎新 A1、億看對照
...
| 項目 | 狀態 |
| --- | --- |
| 產品殼、100 產業包、先選行業、今天一件事、A1 下一步、黑白、海面到海底、折射玻璃、OpenReply 示範回覆 | 可看 |
| ERPNext 過帳、主檔權威 | 未接 |
| Keycloak 登入 | 未接 |
